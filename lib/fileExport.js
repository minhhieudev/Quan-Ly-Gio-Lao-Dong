...
export const exportTongHopLaoDongForUser = async (data, user, namHoc, kiemNhiem) => {
  ...
  // Phân loại chức vụ
  const chucVuChinhQuyen = user?.kiemNhiem?.filter(cv => cv.loaiCV === 'Chính quyền')
    .map(cv => cv.tenCV)
    .join(', ');

  const chucVuDoanThe = user?.kiemNhiem?.filter(cv => ['Đoàn thể', 'Đoàn hội', 'Đảng', 'Công đoàn'].includes(cv.loaiCV))
    .map(cv => cv.tenCV)
    .join(', ');

  const chucVuKiemNhiem = user?.kiemNhiem?.filter(cv => !['Chính quyền', 'Đoàn thể', 'Đoàn hội', 'Đảng', 'Công đoàn'].includes(cv.loaiCV))
    .map(cv => cv.tenCV)
    .join(', ');
  ...
  worksheet.addRow([`KHOA ${user || ''}`, '', '', '', '', '', '', 'Độc lập - Tự do - Hạnh phúc']);
  ...
  worksheet.addRow(['', 'Chức vụ chính quyền:', chucVuChinhQuyen || 'Không', '', '', '', 'Học hàm, học vị:', data.info.userInfo.hocHamHocVi || '']);
  worksheet.mergeCells('C9:F9');
  worksheet.mergeCells('H9:I9');

  worksheet.addRow(['', 'Chức vụ kiêm nhiệm:', chucVuKiemNhiem || 'Không', '', '', '', 'Định mức giờ chuẩn:', data.info.maNgachInfo.GCGD || 0]);
  ...
  worksheet.addRow(['', 'Chức vụ đoàn thể CT - XH:', chucVuDoanThe || '']);
  ...
  const gioChuan = dinhMucChuan - kiemNhiem;
  const dinhMucPhaiThucHien = dinhMucChuan - (tongKiemNhiem || 0);
  const thuaThieuGio = Math.abs(gioChuan - tongGioThucHien);
...
export const exportPcCoiThi = async (dataList, ky, namHoc, loaiKyThi) => {
  ...
  // === 1) Tiêu đề lớn ở dòng 1 (merge A1:K1) ===
  ...
  // Dòng 2 trống để tách biệt
  worksheet.getRow(2).height = 6;

  // === 2) Khai báo cột (chỉ key + width, KHÔNG để header ở đây) ===
  worksheet.columns = [
    { key: "stt", width: 5 },
    { key: "maHocPhan", width: 12 },
    { key: "hocPhan", width: 30 },
    { key: "lop", width: 18 },
    { key: "ngayThi", width: 14 },
    { key: "ca", width: 8 },
    { key: "phong", width: 12 },
    { key: "cbo1", width: 20 },
    { key: "cbo2", width: 20 },
    { key: "hinhThuc", width: 8 },
    { key: "thoiGian", width: 8 },
  ];

  // === 3) Chèn header thủ công ở DÒNG 3 ===
  const headers = [
    "STT",
    "Mã HP",
    "Học phần",
  ...
  worksheet.insertRow(3, headers); // chèn header vào row 3
  const headerRow = worksheet.getRow(3);
  headerRow.font = { bold: true };
  headerRow.alignment = { horizontal: "center", vertical: "middle" };
  headerRow.height = 22;

  // === 4) Thêm dữ liệu (bắt đầu từ row 4) ===
  dataList.forEach((item, index) => {
    // Chuyển ngày (nếu có) sang Date để Excel hiểu là ngày
    const ngayThiDate = item.ngayThi ? new Date(item.ngayThi) : null;
    ...
    // Nếu có ngày thì đặt định dạng ngày (cột E => index 5)
    if (ngayThiDate) {
      const dateCell = row.getCell(5); // cột E là cột thứ 5
      dateCell.numFmt = "dd/mm/yyyy";
    }

    // Căn chỉnh mặc định cho dòng dữ liệu
    row.alignment = { vertical: "middle", horizontal: "left" };
    // Căn giữa các cột cần thiết (A, E, F, G, J, K)
    [1, 5, 6, 7, 10, 11].forEach((colIndex) => {
      row.getCell(colIndex).alignment = { vertical: "middle", horizontal: "center" };
    });
  });

  // === 5) Thêm border cho header + data (từ row 3 trở xuống) ===
  ...
  // Freeze header (giữ 3 dòng trên cùng)
  worksheet.views = [{ state: "frozen", ySplit: 3 }];

  // Tùy chọn: bật filter cho header
...
export const exportTongHopLaoDong = async (dataList, type, title, namHoc, selectedKhoa = '') => {
  ...
  dataList.forEach(item => {
    ...
    const thuaThieu = (tongCong - (item.gioChuan - item.kiemNhiem)) || 0;
  ...
  const dateRow = worksheet.addRow(['', '', '', '', '', '', '', '', '', '', '', `Đắk Lắk, ngày....... tháng ...... năm ${currentYear}`]);
  // ...existing code...
