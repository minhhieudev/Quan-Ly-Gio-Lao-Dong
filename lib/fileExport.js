...
export const exportHocPhan = async (dataList) => {
  ...
  worksheet.columns = [
    ...
    { header: 'Mã MH', key: 'maMH', width: 15 },
    { header: 'Tên MH', key: 'tenMH', width: 30 },
    { header: 'Số TC', key: 'soTC', width: 10 },
  ...
  anchor.href = url;
  anchor.download = 'DanhSachHocPhan.xlsx';
  anchor.click();
...
export const exportTongHopLaoDongForUser = async (data, khoa, namHoc, kiemNhiem) => {
  if (!data || !data.info) {
    console.error("No data available to export");
    return;
  }
  ...
  worksheet.spliceRows(1, 0,
    ['TRƯỜNG ĐẠI HỌC PHÚ YÊN', '', '', '', 'CỘNG HOÀ XÃ HỘI CHỦ NGHĨA VIỆT NAM'],
    ['Khoa '+ khoa, '', '', '', 'Độc lập - Tự do - Hạnh phúc'],
    [''],
  ...
  worksheet.addRow(['', 'Chức vụ kiêm nhiệm:', data.info.kiemNhiemInfo.join(', '), '', '', 'Định mức giờ chuẩn:',  data.info.maNgachInfo.GCGD - kiemNhiem || 0, '', '', '', '', '']);
  ...
  worksheet.addRow(['', 'Tổng cộng', '', '', kiemNhiem, '']);
  ...
  const tongKet = [
    ...
    ['3', 'Tổng số tiết kiêm nhiệm (Công tác kiêm nhiệm)', kiemNhiem],
    ...
    ['', 'TỔNG CỘNG', tongGiangDay.tongCong + tongChamThi + kiemNhiem + tongCoiThi + tongRaDe + tongHuongDan]
  ];
