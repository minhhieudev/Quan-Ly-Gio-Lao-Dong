...
export const exportTongHopLaoDongForUser = async (data, user, namHoc, kiemNhiem) => {
  ...
  // Table Header
  const giangDayHeader1 = worksheet.addRow(['TT', 'Học phần giảng dạy', 'Học kỳ', 'Số TC', 'Lớp học phần', 'Số SV', 'Số tiết thực dạy', '', '', 'Số tiết quy chuẩn', '', 'Tổng cộng', 'Ghi chú']);
  const giangDayHeader2 = worksheet.addRow(['', '', '', '', '', '', 'LT', 'TH', 'BT', 'LT', 'TH', '', '']);
  ...
  worksheet.mergeCells(`G${startHeaderRow}:I${startHeaderRow}`);
  worksheet.mergeCells(`J${startHeaderRow}:K${startHeaderRow}`);
  worksheet.mergeCells(`L${startHeaderRow}:L${startHeaderRow + 1}`);
  worksheet.mergeCells(`M${startHeaderRow}:M${startHeaderRow + 1}`);
  ...
  data.data.CongTacGiangDay.forEach(item => {
    const row = worksheet.addRow([
      ...
      item.soTietLT || 0,
      item.soTietTH || 0,
      0, // BT is missing in data, assuming 0
      item.soTietQCLT || 0,
      item.soTietQCTH || 0,
      item.tongCong,
      item.ghiChu
  ...
  const totalGiangDayRow = worksheet.addRow(['', 'Tổng cộng', '', '', '', '', tongGiangDay.soTietLT, tongGiangDay.soTietTH, 0, tongGiangDay.soTietQCLT, tongGiangDay.soTietQCTH, tongGiangDay.tongCong, '']);
