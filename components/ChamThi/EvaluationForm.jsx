...
const EvaluationForm = ({
  ...
  const fetchCongTacChamThi = async () => {
    try {
  ...
  // Effect để chạy khi component mount và khi có thay đổi từ prop onDataChange
  useEffect(() => {
    if (onDataChange) {
      // Gán hàm fetch data cho callback onDataChange
      onDataChange.current = fetchCongTacChamThi;
    }
  }, [onDataChange]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          `/api/giaovu/pc-cham-thi/get-for-gv/?namHoc=${namHoc}&gvGiangDay=${currentUser?.username}`,
          {
            method: "GET",
            headers: { "Content-Type": "application/json" },
          }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "soTietQuyChuan",
      render: (text) => (
        <span className="text-green-600 font-bold">{text}</span>
      ),
      width: "10%",
      align: "center",
    },
  ...
  const handleSelectChange = (value) => {
    const selectedHocPhan = listSelect.find(
      (item) => item.hocPhan.toLowerCase() === value.toLowerCase()
    );
    if (selectedHocPhan) {
  ...
  const handleSelectChange2 = (value) => {
    const selected = listOptions.find(
      (item) => item?.ten.toLowerCase() == value?.toLowerCase()
    );
          ...
          >
            ...
            <div className="flex justify-between items-center gap-4 bg-gray-50 p-2 rounded-lg mb-1">
              <Form.Item
                label={
                  <span className="font-semibold text-base text-gray-700">
                    Cán bộ chấm thi <span className="text-red-600">*</span>
                  </span>
                }
            ...
            <div className="flex justify-between gap-4 bg-gray-50 p-2 rounded-lg mb-1">
              <Form.Item
                label={
                  <span className="font-semibold text-base text-gray-700">
                    Thời gian (phút) <span className="text-red-600">*</span>
                  </span>
                }
                className="w-1/2 mb-0"
              ...
              <Form.Item
                label={
                  <span className="font-semibold text-base text-gray-700">
                    Hình thức thi <span className="text-red-600">*</span>
                  </span>
                }
                className="w-1/2 mb-0"
            ...
            <div className="flex justify-between gap-4 bg-gray-50 p-2 rounded-lg mb-1">
              <Form.Item
                label={
                  <span className="font-semibold text-base text-gray-700">
                    Số bài chấm <span className="text-red-600">*</span>
                  </span>
                }
                className="w-1/2 mb-0"
            ...
            },
            {
              ...
              children: loadings ? (
                ...
                <TablePcChamThi
                  ...
                  type={type}
                  onDataChange={fetchCongTacChamThi}
                  onSwitchTab={() => setSelectedTab("Kết quả chấm thi")} // ✅ thêm dòng này
                />
              ),
            },
          ]}
