...
const ExamMonitoringForm = ({ onUpdateCongTacCoiThi, namHoc, ky }) => {
  ...
  useEffect(() => {
    ...
    if (
      currentHocPhan?.time &&
      Array.isArray(currentHocPhan?.time) &&
      currentHocPhan?.time.length > 0
    ) {
      timeValue =
        currentHocPhan?.time.length > 1
  ...
  const handleSelectChange = (setCurrentHocPhan) => {
    const selectedHocPhan = listSelect.find(
      (item) => item.hocPhan.join(", ") === setCurrentHocPhan
    );
    ...
    if (selectedHocPhan) {
      ...
      if (
        Array.isArray(selectedHocPhan.thoiGian) &&
        selectedHocPhan.thoiGian.length > 0
      ) {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(
        `/api/pc-coi-thi?namHoc=${namHoc}&userName=${encodeURIComponent(
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (
          editRecord ||
          dataList.some((item) => item.hocPhan === newData.hocPhan)
        ) {
          setDataList((prevData) =>
            prevData.map((item) =>
              item.hocPhan === newData.hocPhan ? newData : item
            )
          );
        } else {
        ...
        try {
            ...
            if (row && row.length > 0) {
              ...
              if (!foundFirstDot) {
                ...
                const isDotRowInHeader =
                  ...
                  (lowerCellText.includes("đợt") &&
                    lowerCellText.includes("học phần")) ||
                  (lowerCellText.includes("đợt") &&
                    lowerCellText.includes("2024-2025")) ||
                  (lowerCellText.includes("đợt") &&
                    lowerCellText.includes("năm học")) ||
                  // THÊM: Nhận diện format "Đợt học kỳ X, đợt Y"
                  (lowerCellText.includes("đợt") &&
                    lowerCellText.includes("học kỳ")) ||
                  // THÊM: Nhận diện bất kỳ dòng nào chứa "đợt" và có số
                  (lowerCellText.includes("đợt") &&
                    /đợt\s*\d+/.test(lowerCellText));
          ...
          for (let i = 0; i < jsonData.length; i++) {
            ...
            if (
              row &&
              row.some(
                (cell) => cell && cell.toString().includes("Tên học phần")
              )
            ) {
              headerRowIndex = i;
              dataStartIndex = i + 1;
          ...
          if (headerRowIndex === -1) {
            toast.error(
              'Không tìm thấy header "Tên học phần" trong file Excel!'
            );
          ...
          const columnIndexes = {
            maHocPhan: headers.findIndex(
              (h) => h && h.toString().includes("Mã học phần")
            ),
            tenHocPhan: headers.findIndex(
              (h) => h && h.toString().includes("Tên học phần")
            ),
            lopHP: headers.findIndex(
              (h) => h && h.toString().includes("Lớp HP")
            ),
            ngayThi: headers.findIndex(
              (h) => h && h.toString().includes("Ngày thi")
            ),
            buoiThi: headers.findIndex(
              (h) => h && h.toString().includes("Buổi thi")
            ),
            gioThi: headers.findIndex(
              (h) => h && h.toString().includes("Giờ thi")
            ),
            thoiGianThi: headers.findIndex(
              (h) =>
                h &&
                (h.toString().includes("Thời gian thi") ||
                  h.toString().includes("Thời gian"))
            ),
            phongThi: headers.findIndex(
              (h) => h && h.toString().includes("Phòng thi")
            ),
            canBoGiangDay: headers.findIndex(
              (h) => h && h.toString().includes("Cán bộ giảng dạy")
            ),
            vaiTro: headers.findIndex(
              (h) => h && h.toString().includes("Vai trò")
            ),
          };
          ...
          if (columnIndexes.tenHocPhan === -1 || columnIndexes.ngayThi === -1) {
            toast.error(
              "Không tìm thấy các cột cần thiết (Tên học phần, Ngày thi) trong file Excel!"
            );
            toast.dismiss("excel-import");
            return;
          }
          ...
          for (let i = dataStartIndex; i < jsonData.length; i++) {
            ...
            const isDotRow =
              ...
              (lowerRowText.includes("đợt") &&
                lowerRowText.includes("học phần")) ||
              (lowerRowText.includes("đợt") &&
                lowerRowText.includes("2024-2025")) ||
              (lowerRowText.includes("đợt") &&
                lowerRowText.includes("năm học"));
            ...
            if (
              ...
              secondCell.includes("mã học phần") ||
              secondCell.includes("tên học phần")
            ) {
              continue;
            }
            ...
            const rowData = {
              maHocPhan:
                columnIndexes.maHocPhan !== -1
                  ? row[columnIndexes.maHocPhan] || ""
                  : "",
              tenHocPhan:
                columnIndexes.tenHocPhan !== -1
                  ? row[columnIndexes.tenHocPhan] || ""
                  : "",
              lopHP:
                columnIndexes.lopHP !== -1
                  ? row[columnIndexes.lopHP] || ""
                  : "",
              ngayThi:
                columnIndexes.ngayThi !== -1
                  ? row[columnIndexes.ngayThi] || ""
                  : "",
              buoiThi:
                columnIndexes.buoiThi !== -1
                  ? row[columnIndexes.buoiThi] || ""
                  : "",
              gioThi:
                columnIndexes.gioThi !== -1
                  ? row[columnIndexes.gioThi] || ""
                  : "",
              thoiGianThi:
                columnIndexes.thoiGianThi !== -1
                  ? row[columnIndexes.thoiGianThi] || ""
                  : "",
              phongThi:
                columnIndexes.phongThi !== -1
                  ? row[columnIndexes.phongThi] || ""
                  : "",
              canBoGiangDay:
                columnIndexes.canBoGiangDay !== -1
                  ? row[columnIndexes.canBoGiangDay] || ""
                  : "",
              vaiTro:
                columnIndexes.vaiTro !== -1
                  ? row[columnIndexes.vaiTro] || ""
                  : "",
            };
            ...
            if (
              !rowData.tenHocPhan ||
              rowData.tenHocPhan.toString().trim() === ""
            ) {
              continue;
            }
            if (!rowData.ngayThi) {
            ...
            if (rowData.tenHocPhan && rowData.ngayThi) {
              ...
              try {
                if (typeof rowData.ngayThi === "number") {
                  ...
                  const excelDate = new Date(
                    (rowData.ngayThi - 25569) * 86400 * 1000
                  );
                  ...
                  if (!isNaN(rowData.ngayThi.getTime())) {
                    formattedNgayThi = rowData.ngayThi
                      .toISOString()
                      .split("T")[0];
                  }
                } else if (
                  typeof rowData.ngayThi === "string" &&
                  rowData.ngayThi.trim() !== ""
                ) {
              ...
              let soTietQuyChuan = calcSoTietQuyChuan(
                rowData.thoiGianThi,
                formattedNgayThi
              );
              ...
              importedData.push({
                ...
                thoiGianThi: rowData.thoiGianThi
                  ? rowData.thoiGianThi.toString().trim()
                  : "",
          ...
          for (let i = dataStartIndex; i < jsonData.length; i++) {
            ...
            if (
              lowerRowData.includes("stt") ||
              lowerRowData.includes("mã học phần") ||
              lowerRowData.includes("tên học phần")
            ) {
              continue;
            }
            ...
            try {
              ...
              console.warn(
                "Error parsing date for PcCoiThi:",
                rowData.ngayThi,
                dateError
              );
            }
            ...
            if (rowData.vaiTro) {
              ...
              if (
                vaiTroStr.includes("cán bộ coi thi 2") ||
                vaiTroStr.includes("cbo2")
              ) {
            ...
            const itemData = {
              ...
              ngayThi: formattedNgayThi,
              ca: ca,
              phong: rowData.phongThi
                ? [rowData.phongThi.toString().trim()]
                : [],
              cbo1: cbo1,
              cbo2: cbo2,
              hinhThuc: [], // Để trống
              thoiGian: rowData.thoiGianThi
                ? [rowData.thoiGianThi.toString().trim()]
                : [],
            ...
            if (currentLoaiKyThi !== itemData.loaiKyThi) {
              console.error(
                "❌ MISMATCH: currentLoaiKyThi !== itemData.loaiKyThi"
              );
          ...
          try {
            ...
            if (congTacRes.ok) {
              ...
              if (results.success && results.success.length > 0) {
                setDataList((prevData) => {
                  const newRecords = results.success.filter(
                    (newRecord) =>
                      !prevData.some(
                        (existing) =>
            ...
            if (hasError) {
              toast.error(
                "Import một phần thành công. Kiểm tra console để xem chi tiết lỗi."
              );
            } else {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return isEditing(record) ? (
          <Form.Item name="soTietQuyChuan" style={{ margin: 0 }}>
    ...
    <div className="flex gap-2 max-sm:flex-col h-full overflow-hidden">
      {showForm && (
        <div className="flex flex-col flex-[30%] ">
          ...
          >
            <span className="flex items-center justify-center">Ẩn Form</span>
          </Button>
          <div className="px-6 h-[100%] py-7 shadow-lg bg-white rounded-xl border border-gray-200 overflow-auto">
            <div className="border-b border-blue-500 pb-2 mb-1">
              <Title className="text-center text-blue-600 !mb-0" level={4}>
                CÔNG TÁC COI THI
              </Title>
            </div>
            <Form
                            ...
                            render={({ field }) => (
                              <Select
                                ...
                                options={listSelect.map((item, index) => ({
                                  value: Array.isArray(item.hocPhan)
                                    ? item.hocPhan.join(", ")
                                    : item.hocPhan,
                                  label: Array.isArray(item.hocPhan)
                                    ? item.hocPhan.join(", ")
                                    : item.hocPhan,
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <Input
                          value={newHocPhan}
                          onChange={(e) =>
                            setNewHocPhan(e.target.value.split(","))
                          }
                ...
                <div className="bg-gray-50 p-3 rounded-lg mb-2">
                  <div className="flex gap-2">
                    ...
                    <Form.Item
                      label={
                        <span className="font-semibold text-base text-gray-700">
                          Ngày thi <span className="text-red-600">*</span>
                        </span>
                      }
                      className="w-full md:w-1/2 mb-2"
                    >
                  ...
                  <div className="flex justify-between items-start gap-2 flex-wrap">
                    ...
                    >
                      <Controller
                        ...
                        control={control}
                        rules={{
                          required: "Số tiết quy chuẩn là bắt buộc",
                          min: {
                            value: 1,
                            message: "Số tiết quy chuẩn phải lớn hơn 0",
                          },
                        }}
                        render={({ field }) => (
                          <InputNumber
                    ...
                    <Upload
                      ...
                      beforeUpload={(file) => {
                        Modal.confirm({
                          ...
                          content: (
                            <span>
                              Bạn có chắc chắn muốn import dữ liệu cho{" "}
                              <b>năm học {namHoc}</b> không?
                            </span>
                          ),
                          okText: "Xác nhận",
                          cancelText: "Hủy",
                          onOk: () => handleImportExcel(file),
                        });
                        // ⚠️ Trả false để chặn upload mặc định của antd
                        return false;
                      }}
                    >
                      <Button
      ...
      <div
        className={`p-6 shadow-lg bg-white rounded-xl text-center border border-gray-200 overflow-y-auto transition-all duration-300 ${
          showForm ? "flex-[75%]" : "flex-[100%] w-full"
        }`}
      >
        <Tabs
          ...
          items={[
            {
              key: "Kết quả coi thi",
              label: (
                <span className="font-semibold text-base">KẾT QUẢ COI THI</span>
              ),
              children: loadings ? (
